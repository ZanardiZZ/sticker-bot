...
if [[ "${DEEPSEEK_MODEL}" == "deepseek-coder:6.7b" ]]; then
  cat >&2 <<'EOF'
codex-deepseek: `deepseek-coder:6.7b` is reachable through Ollama, but Codex OSS rejected it because the model does not support tools.
...
exec codex --oss --local-provider ollama -m "${DEEPSEEK_MODEL}" "$@"
