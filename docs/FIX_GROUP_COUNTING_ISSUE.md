...
```javascript
// BEFORE (INCORRECT)
const chatId = message.from;  // "1203630000@g.us" (group ID) ❌
const groupId = message.from.endsWith('@g.us') ? message.from : null;
const senderId = resolvedSenderId;  // "5511000000000@c.us" (user ID) ✓

await saveMedia({
  chatId,    // "1203630000@g.us" ❌
  groupId,   // "1203630000@g.us" ✓
  senderId   // "5511000000000@c.us" ✓
});
```
...
const groupId = message.from.endsWith('@g.us') ? message.from : null;
const senderId = resolvedSenderId;  // "5511000000000@c.us"
const chatIdForDb = groupId ? senderId : chatId;  // ✓ FIX

await saveMedia({
  chatId: chatIdForDb,  // "5511000000000@c.us" ✓
  groupId,              // "1203630000@g.us" ✓
  senderId              // "5511000000000@c.us" ✓
});
```
...
1. **Send a test sticker in a group**
   ```
   User sends sticker in group "1203630000@g.us"
   ```
...
4. **Verify database (optional)**
   ...
   Expected result:
   - `chat_id`: User ID (e.g., "5511000000000@c.us")
   - `group_id`: Group ID (e.g., "1203630000@g.us")
   - `sender_id`: User ID (e.g., "5511000000000@c.us")
