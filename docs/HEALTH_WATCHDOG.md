...
## Arquivos
- Script: `<PROJECT_ROOT>/scripts/ops/health-watchdog.js`
- Log JSONL: `<PROJECT_ROOT>/storage/logs/health-watchdog.log`
- Estado de leitura incremental do log: `<PROJECT_ROOT>/storage/logs/health-watchdog.state.json`
- Log da execução via cron: `<PROJECT_ROOT>/storage/logs/health-watchdog-cron.log`
...
```cron
*/2 * * * * cd <PROJECT_ROOT> && /usr/bin/env node scripts/ops/health-watchdog.js >> <PROJECT_ROOT>/storage/logs/health-watchdog-cron.log 2>&1
```
...
- `HEALTH_ALERT_WHATSAPP_JID` (destino prioritário de alerta, ex: `5511000000000@c.us` ou `1203...@g.us`)
