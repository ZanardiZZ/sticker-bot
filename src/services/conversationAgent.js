...
function buildMemoryPrompt(memoryContext = {}, senderId = null) {
  const sections = [];
  const user = senderId && memoryContext.users ? memoryContext.users[senderId] : null;
  ...
  const runningJokes = Array.isArray(memoryContext.runningJokes) ? memoryContext.runningJokes : [];
  const activeTopics = Array.isArray(memoryContext.activeTopics) ? memoryContext.activeTopics : [];
  const groupDynamics = Array.isArray(memoryContext.groupDynamics) ? memoryContext.groupDynamics : [];
...
function buildAiMessages(state, { groupName, memoryContext = null, senderId = null }) {
  ...
  return [
    { role: 'system', content: `${buildSystemPrompt(groupName)} ${roster} ${memoryPrompt}`.trim() },
    ...dialogue
  ];
}
...
async function handleGroupChatMessage(client, message, context = {}) {
  try {
    ...
    if (!memoryContext && memory.isReady()) {
