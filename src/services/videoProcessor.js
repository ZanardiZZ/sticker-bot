...
async function extractFrames(filePath, timestamps) {
  ...
  let tempDir = path.join(TEMP_DIR, `frames_${processId}_${uniqueId}`);
  ...
  try {
    ...
    const altTempDir = path.join(TEMP_DIR, `frames_alt_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`);
    try {
...
async function extractAudio(filePath) {
  ...
  const uniqueId = crypto.randomBytes(16).toString('hex');
  const output = path.join(TEMP_DIR, `audio_${uniqueId}.wav`);
  return new Promise((resolve, reject) => {
    let ffmpegProcess = null;
    const timeoutId = setTimeout(() => {
      if (ffmpegProcess) {
...
async function processAnimatedWebp(filePath) {
  ...
  try {
    ...
    const tempDir = path.join(TEMP_DIR, `webp_frames_${processId}_${uniqueId}`);
