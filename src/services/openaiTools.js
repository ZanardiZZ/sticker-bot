...
const { DB_PATH, LOGS_DIR } = require('../paths');
  ...
  return [
...

    {
      ...
      function: {
        ...
        description: 'Modifica valores de configuração do bot na tabela bot_config. Use para corrigir configs perdidas ou incorretas.',
        parameters: {
          type: 'object',
          properties: {
            key: {
...
async function getBotLogs({ service, lines = 50, level = 'all' }) {
  ...
  const logFiles = {
    'bot': path.join(LOGS_DIR, 'bot-out.log'),
    'baileys': path.join(LOGS_DIR, 'baileys-out.log'),
    'web': path.join(LOGS_DIR, 'web-out.log')
  };
...
async function getSchedulerStatus() {
  try {
    ...
    return new Promise((resolve, reject) => {
      // Check if scheduler is enabled in bot_config
      db.get(`
      ...
      `, (err, row) => {
        ...
        `, (err2, lastSent) => {
          ...
          // Get cron expression
          db.get(`
            SELECT value FROM bot_config WHERE key = 'auto_send_cron'
          `, (err3, cronRow) => {
...
async function readFile({ filePath, lines = 100 }) {
  try {
    ...
    const forbiddenPaths = ['auth_info_baileys', 'storage/auth_info_baileys', 'node_modules/.', '.git/'];
...
async function writeFileContent({ filePath, content, append = false }) {
  try {
    ...
    const forbiddenPaths = ['.env', 'auth_info_baileys', 'storage/auth_info_baileys', 'node_modules', '.git', 'media.db', 'storage/database/media.db'];
