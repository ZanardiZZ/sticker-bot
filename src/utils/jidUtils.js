/**
 * Decodifica um JID em suas partes componentes (implementação própria, sem dependência do Baileys)
 * @param {string} jid - O JID a ser decodificado
 * @returns {Object|null} Objeto com user, server, device, etc ou null se inválido
 */
function jidDecode(jid) {
    ...
    const [userPart, server] = parts;
    const deviceParts = userPart.split(':');

    return {
        user: deviceParts[0],
        server: server,
        device: deviceParts[1] ? parseInt(deviceParts[1]) : undefined,
        agent: deviceParts.length > 2 ? parseInt(deviceParts[2]) : undefined
    };
}
