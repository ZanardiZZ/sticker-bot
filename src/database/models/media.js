  ...
  return new Promise((resolve, reject) => {
    ...
    database.get(
      `WITH inferred_mapping AS (
       ...
       -- Resolve the input senderId to the same canonical form used by effective_sender.
       -- If senderId is a LID, map it to its PN (via lid_mapping or inferred_mapping);
       -- the query then matches both the raw senderId and its resolved form.
       input_ids AS (
         SELECT ? AS raw_id,
           CASE
             WHEN ? LIKE '%@lid' THEN
               COALESCE(
                 NULLIF((SELECT pn FROM lid_mapping WHERE lid = ? LIMIT 1), ''),
                 (SELECT pn FROM inferred_mapping WHERE lid = ? LIMIT 1),
                 ?
               )
             ELSE ?
           END AS resolved_id
       ...
       FROM resolved, input_ids
       WHERE effective_sender = input_ids.raw_id
          OR effective_sender = input_ids.resolved_id`,
      [senderId.trim(), senderId.trim(), senderId.trim(), senderId.trim(), senderId.trim(), senderId.trim()],
      (err, row) => {
        if (err) {
          reject(err);
        } else {
