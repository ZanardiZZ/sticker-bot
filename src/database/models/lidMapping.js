...
// In-memory LID↔PN cache to avoid repeated DB lookups (TTL: 10 minutes)
const LID_CACHE_TTL = 10 * 60 * 1000;
const lidToPnCache = new Map(); // lid -> { pn, ts }
const pnToLidCache = new Map(); // pn -> { lid, ts }

function cacheSet(lid, pn) {
    if (!lid || !pn) return;
    const ts = Date.now();
    lidToPnCache.set(lid, { pn, ts });
    pnToLidCache.set(pn, { lid, ts });
}

function cacheGetPn(lid) {
    const entry = lidToPnCache.get(lid);
    if (!entry) return null;
    if (Date.now() - entry.ts > LID_CACHE_TTL) { lidToPnCache.delete(lid); return null; }
    return entry.pn;
}

function cacheGetLid(pn) {
    const entry = pnToLidCache.get(pn);
    if (!entry) return null;
    if (Date.now() - entry.ts > LID_CACHE_TTL) { pnToLidCache.delete(pn); return null; }
    return entry.lid;
}
...
function storeLidPnMapping(lid, pn) {
    ...
    // Update cache immediately (avoids DB roundtrip on next lookup)
    cacheSet(normalizedLid, normalizedPn);
...
function getPnForLid(lid) {
    ...
    const cached = cacheGetPn(normalized);
            ...
            db.get(
                ...
                (err, result) => {
                    if (err) {
                        ...
                        const pn = result?.pn || null;
                        if (pn) cacheSet(normalized, pn);
                        resolve(pn);
                    }
                }
            );
...
function getLidForPn(pn) {
    ...
    // Check in-memory cache first
    const cached = cacheGetLid(normalized);
    if (cached) return Promise.resolve(cached);
            ...
            db.get(
                ...
                (err, result) => {
                    if (err) {
                        ...
                        if (lid) cacheSet(lid, normalized);
                        resolve(lid);
                    }
                }
            );
