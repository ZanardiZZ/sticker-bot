...
async function loadReactionAnalytics(){const status=document.getElementById('reactionAnalyticsStatus'),body=document.getElementById('reactionAnalyticsBody');if(!status||!body)return;try{const r=await fetch('/api/admin/reactions/analytics?days=30',{credentials:'same-origin',cache:'no-store'});if(!r.ok)throw new Error('HTTP '+r.status);const d=await r.json();const trends=d.trends||{};const renderTrend=(title,rows)=>'<strong>'+title+'</strong><br>'+(rows||[]).map(x=>x.period+' — '+x.count).join('<br>')||'<em>sem dados</em>';status.textContent='Últimos 30 dias — '+(d.totalReactions||0)+' reações';body.innerHTML='<strong>Top stickers</strong><br>'+(d.topMedia||[]).map((m,i)=>(i+1)+'. ID #'+m.media_id+' — '+m.reaction_count).join('<br>')+'<br><br><strong>Emojis</strong><br>'+(d.emojiCounts||[]).map(e=>e.emoji+' × '+e.count).join('  ')+'<br><br>'+renderTrend('Tendência semanal',trends.weekly)+'<br><br>'+renderTrend('Tendência mensal',trends.monthly);}catch(e){status.textContent='Não foi possível carregar analytics agora.';body.textContent='';}}
