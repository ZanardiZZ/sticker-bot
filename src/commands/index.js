...
// In-memory cache for group command permissions (TTL: 5 min)
const _groupPermCache = new Map();
const GROUP_PERM_TTL = 5 * 60 * 1000;
...
async function isGroupCommandAllowed(groupId, command, senderId) {
  ...
  const cached = _groupPermCache.get(cacheKey);
  if (cached && Date.now() - cached.ts < GROUP_PERM_TTL) {
  ...
  try {
    ...
    const allowed = !row ? true : row.allowed !== 0;
    _groupPermCache.set(cacheKey, { allowed, ts: Date.now() });
    return allowed;
  } catch (err) {
...
async function handleCommand(client, message, chatId, context = {}) {
  ...
  if (handled && shouldTrackUsage) {
    ...
    if (usageUserId && typeof incrementCommandUsage === 'function') {
      // Fire-and-forget: don't block command response waiting for analytics write
      incrementCommandUsage(command, usageUserId).catch(incrementError => {
