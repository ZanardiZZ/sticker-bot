...
const tests = [
  ...
  
  {
    ...
    fn: async () => {
      delete process.env.ADMIN_NUMBERS;
      delete process.env.BOT_SUPER_ADMINS;
      process.env.ADMIN_NUMBER = '5511000000000@c.us';
      
      const result = getEnvAdminSet();
      
      assertEqual(result.size, 1, 'Should have one admin');
      assert(result.has('5511000000000@c.us'), 'Should contain the admin number');
  ...
  
  {
    ...
    fn: async () => {
      delete process.env.ADMIN_NUMBER;
      delete process.env.BOT_SUPER_ADMINS;
      process.env.ADMIN_NUMBERS = '5511000000000@c.us, 5511888888888@c.us, 5511777777777@c.us';
      
      const result = getEnvAdminSet();
      
      assertEqual(result.size, 3, 'Should have three admins');
      assert(result.has('5511000000000@c.us'), 'Should contain first admin');
  ...
  
  {
    ...
    fn: async () => {
      process.env.ADMIN_NUMBER = '5511000000000@c.us';
      process.env.ADMIN_NUMBERS = '5511888888888@c.us, 5511000000000@c.us'; // Duplicate
      ...
      assert(result.has('5511000000000@c.us'), 'Should contain admin from ADMIN_NUMBER');
  ...
  
  {
    ...
    fn: async () => {
      delete process.env.ADMIN_NUMBERS;
      delete process.env.BOT_SUPER_ADMINS;
      process.env.ADMIN_NUMBER = '5511000000000'; // Without @c.us
      
      const result = getEnvAdminSet();
      
      assertEqual(result.size, 1, 'Should have one admin');
      // normalizeJid should add @c.us suffix
      assert(result.has('5511000000000@c.us') || result.has('5511000000000'), 'Should normalize the JID');
  ...
  
  {
    ...
    fn: async () => {
      delete process.env.ADMIN_NUMBER;
      delete process.env.BOT_SUPER_ADMINS;
      process.env.ADMIN_NUMBERS = '5511000000000@c.us, , 5511888888888@c.us, '; // Empty entries
