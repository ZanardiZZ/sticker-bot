...
const { getReactionNotificationConfig, shouldNotifyReactionMilestone } = require('../../src/services/reactionNotifications');
...
const tests=[
 ...
 {name:'reaction notifications are opt-in and milestone based',fn:async()=>{const config=getReactionNotificationConfig({REACTION_NOTIFICATIONS_ENABLED:'1',REACTION_NOTIFICATION_THRESHOLD:'5'});assertEqual(config.enabled,true);assertEqual(config.threshold,5);const key='test-reaction-milestone';assertEqual(shouldNotifyReactionMilestone({count:5,key,...config,now:100000}),true);assertEqual(shouldNotifyReactionMilestone({count:5,key,...config,now:100001}),false);assertEqual(shouldNotifyReactionMilestone({count:6,key,...config,now:100002}),false);}},
