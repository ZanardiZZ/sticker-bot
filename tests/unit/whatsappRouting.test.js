...
const tests = [
  {
    ...
    fn: async () => {
      const result = parseJidCsv(' 120363000000000000@g.us , 120363420825912004@g.us,120363000000000000@g.us ');
      assert(result instanceof Set, 'Should return a Set');
      assertEqual(result.size, 2, 'Should deduplicate JIDs');
      assert(result.has('120363000000000000@g.us'), 'Should contain GT group');
  ...
  },
  {
    ...
    fn: async () => {
      ...
      process.env.AUTO_SEND_GROUP_ID = '120363000000000000@g.us';
      ...
      assert(result.has('120363000000000000@g.us'), 'Should include auto-send group');
  ...
  },
  {
    ...
    fn: async () => {
      const allowed = new Set(['120363000000000000@g.us']);
      assert(isJidAllowed('120363000000000000@G.US', allowed), 'Should match normalized JID');
