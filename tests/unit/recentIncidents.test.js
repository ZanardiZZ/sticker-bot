...
const read=(p)=>{const target=path.join(root,p);if(fs.existsSync(target))return fs.readFileSync(target,'utf8');if(p==='\.env')return fs.readFileSync(path.join(root,'.env.example'),'utf8');return fs.readFileSync(target,'utf8');};
const tests=[
...
{name:'memory context is enabled and compatibility methods exist',fn:async()=>{const env=read('.env');const s=read('src/client/memory-client.js');assert(/CONVERSATION_ENABLE_MEMORY_CONTEXT=[01]/.test(env));assert(s.includes('async ensureUser')&&s.includes('async ensureGroup'));}},
