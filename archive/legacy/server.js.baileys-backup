...
// Format: country code + number without + or spaces (e.g., 5511000000000)
...
async function start() {
  ...
  const version = [2, 3000, 1034785817]; // Latest WA Web alpha (2026-03-09 10:24PM) - https://wppconnect.io/whatsapp-versions/
  ...
  const sock = makeWASocket({
    ...
    browser: ['Mac OS', 'Desktop', '1.0'], // Changed to match MACOS platform (PR #2365)
    countryCode: 'BR', // Brazil country code
  ...
  if (shouldUsePairing) {
    if (!PAIRING_PHONE_NUMBER) {
      ...
      console.error('[Baileys] Example: BAILEYS_PAIRING_PHONE_NUMBER=5511000000000');
      process.exit(1);
    }
