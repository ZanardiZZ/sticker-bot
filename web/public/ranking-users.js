(function () {
  function $(sel) { return document.querySelector(sel); }
  function pick(...vals) { for (const v of vals) if (v !== undefined && v !== null && v !== '') return v; }

  function resolveRowsPayload(d) {
    if (Array.isArray(d)) return d;
    if (d && Array.isArray(d.rows)) return d.rows;
    if (d && Array.isArray(d.data)) return d.data;
    return [];
  }

  async function fetchJSON(url) {
    const r = await fetch(url, { credentials: 'same-origin' });
    if (!r.ok) {
      let msg = 'HTTP ' + r.status;
      try { const j = await r.json(); if (j?.error) msg += ' - ' + j.error; if (j?.message) msg += ' - ' + j.message; } catch {}
      throw new Error(msg);
    }
    return r.json();
  }

  // Gera um rótulo sem expor o telefone. Ex: "Usuário ••95"
  function maskJid(jid) {
    if (!jid) return 'Usuário';
    const local = String(jid).split('@')[0] || '';
    const digits = local.replace(/\D/g, '');
    const last2 = digits.slice(-2);
    return 'Usuário ' + (last2 ? '••' + last2 : '••');
  }

  function ensureMsgBox(parent) {
    let box = $('#users_msg');
    if (!box) {
      box = document.createElement('div');
      box.id = 'users_msg';
      box.style.margin = '8px 0';
      box.style.color = '#bbb';
      parent.prepend(box);
    }
    return box;
  }

  function ensureUsersTable() {
    let tbody =
      $('#tblUsers tbody') ||
      $('#users tbody') ||
      document.querySelector('table#tblUsers tbody') ||
      document.querySelector('table#users tbody');

    if (tbody) return { tbody, isTable: true, parent: tbody.closest('table').parentElement };

    let list =
      $('#users-list') || $('#users') || $('#list');
    if (list) return { tbody: list, isTable: false, parent: list.parentElement };

    const parent = $('#content') || $('main') || $('.container') || $('.dash') || document.body;
    const wrap = document.createElement('div');
    wrap.id = 'users_auto_container';
    const table = document.createElement('table');
    table.id = 'tblUsers';
    table.style.width = '100%';
    table.style.borderCollapse = 'collapse';
    const thead = document.createElement('thead');
    thead.innerHTML = '<tr><th style="text-align:left;padding:.4rem .5rem;border-bottom:1px solid #333;">#</th><th style="text-align:left;padding:.4rem .5rem;border-bottom:1px solid #333;">Usuário</th><th style="text-align:left;padding:.4rem .5rem;border-bottom:1px solid #333;">Qtd</th></tr>';
    const tb = document.createElement('tbody');
    table.appendChild(thead);
    table.appendChild(tb);
    wrap.appendChild(table);
    parent.appendChild(wrap);
    console.log('[rank/users] tabela criada automaticamente.');
    return { tbody: tb, isTable: true, parent: wrap };
  }

  function showMessage(parent, text, isError) {
    const box = ensureMsgBox(parent);
    box.textContent = text || '';
    box.style.color = isError ? '#f88' : '#bbb';
  }

  async function load() {
    const { tbody, isTable, parent } = ensureUsersTable();
    try {
      const params = new URLSearchParams(location.search);
      const nsfw = (params.get('nsfw') || 'all');
      const limit = Number(params.get('limit') || 50);
      const url = `/api/rank/users?nsfw=${encodeURIComponent(nsfw)}&limit=${limit}`;

      const data = await fetchJSON(url);
      console.log('[rank/users] payload:', data);
      const rows = resolveRowsPayload(data);

      if (!rows.length) {
        if (isTable) tbody.innerHTML = '';
        showMessage(parent, 'Nenhum usuário encontrado para os filtros atuais.');
        return;
      }

      if (isTable) {
        const html = rows.map((r, i) => {
          const rawId = pick(r.sender_id, r.chat_id, r.user_id, r.id, '');
          const display = pick(r.display_name, r.name, r.username) || maskJid(rawId);
          const count = pick(r.count, r.sticker_count, r.total, r.value, 0);
          return `<tr>
            <td style="padding:.4rem .5rem;border-bottom:1px solid #333;">${i + 1}</td>
            <td style="padding:.4rem .5rem;border-bottom:1px solid #333;">${display}</td>
            <td style="padding:.4rem .5rem;border-bottom:1px solid #333;">${count}</td>
          </tr>`;
        }).join('');
        tbody.innerHTML = html;
      } else {
        tbody.innerHTML = rows.map((r, i) => {
          const rawId = pick(r.sender_id, r.chat_id, r.user_id, r.id, '');
          const display = pick(r.display_name, r.name, r.username) || maskJid(rawId);
          const count = pick(r.count, r.sticker_count, r.total, r.value, 0);
          return `<div class="row"><span>${i + 1}.</span> <strong>${display}</strong> <span>${count}</span></div>`;
        }).join('');
      }
      showMessage(parent, '');
    } catch (e) {
      console.error('[rank/users] erro:', e);
      showMessage(parent, 'Erro ao carregar ranking de usuários: ' + e.message, true);
    }
  }

  document.addEventListener('DOMContentLoaded', load);
})();